...
inner join [Dimension].[Customer] on [Fact].[Sale].[Sale Key]=[Dimension].[Customer].[Customer Key]
inner join [Dimension].[City] on [Fact].[Sale].[Sale Key]=[Dimension].[City].[City Key]
order by Profit asc
 go
...
select  top 5 * from [dbo].[NewDimensionCustomer] /*Selecting top 105rows*/
...
select distinct* from [Fact].[Sale]
select distinct * from [Fact].[Order]




--21)Co-Related Sub Queries
